...
export default function RoutinesPage() {
  ...
  return <div className="space-y-6">
    ...
    <div className={`grid gap-3 rounded-xl border border-slate-200 bg-white p-4 shadow-sm print:hidden md:grid-cols-2 ${isTeacherScope ? 'xl:grid-cols-4' : 'xl:grid-cols-5'}`}>
      ...
      {!isTeacherScope && <Select label="Teacher" value={teacherFilter} onChange={setTeacherFilter} placeholder="All teachers" options={options.teachers.map((item) => ({ value: item.id, label: `${item.nameEn} (${item.employeeCode})` }))} />}
    ...
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><h2 className="flex items-center gap-2 text-sm font-bold"><Clock className="h-4 w-4 text-teal-600" />Weekly timetable</h2><span className="text-xs text-slate-500">{data.length} slots</span></div>
      {loading ? <div className="space-y-3 p-6 animate-pulse"><div className="h-10 rounded bg-slate-100" /><div className="h-14 rounded bg-slate-50" /></div> : !data.length ? <DatabaseEmptyState title={isTeacherScope ? "No assigned class routine" : "No class routine"} description={isTeacherScope ? "No routine slot is currently assigned to your teacher profile." : "Use Add Routine Slot to build the weekly timetable."} /> : <div className="overflow-x-auto"><table className="w-full min-w-[950px] text-left text-xs"><thead className="bg-slate-50 text-slate-500"><tr><th className="p-3">Day / Period</th><th className="p-3">Time</th><th className="p-3">Class</th><th className="p-3">Group</th><th className="p-3">Subject</th>{!isTeacherScope && <th className="p-3">Teacher</th>}<th className="p-3">Room</th><th className="p-3">Status</th>{canManage && <th className="p-3 print:hidden">Actions</th>}</tr></thead><tbody className="divide-y divide-slate-100">{data.map((item) => <tr key={item.id} className="hover:bg-slate-50"><td className="p-3"><p className="font-bold">{item.weekday}</p><p className="text-[10px] text-slate-400">{item.periodName}</p></td><td className="p-3 font-mono">{item.startTime}–{item.endTime}</td><td className="p-3"><p className="font-bold">{item.className}</p><p className="text-[10px] text-slate-400">{item.sectionName}</p></td><td className="p-3">{item.groupName || 'All'}</td><td className="p-3"><p className="font-semibold">{item.subjectName}</p><p className="text-[10px] text-slate-400">{item.subjectCode}</p></td>{!isTeacherScope && <td className="p-3"><p>{item.teacherName}</p><p className="text-[10px] text-slate-400">{item.teacherCode}</p></td>}<td className="p-3">{item.roomName}</td><td className="p-3"><StatusBadge status={item.status} /></td>{canManage && <td className="p-3 print:hidden"><div className="flex gap-1"><button onClick={() => openEdit(item)} title="Edit" className="rounded-md p-2 text-blue-600 hover:bg-blue-50"><Edit2 className="h-4 w-4" /></button><button onClick={() => setDeleting(item)} title="Delete" className="rounded-md p-2 text-rose-600 hover:bg-rose-50"><Trash2 className="h-4 w-4" /></button></div></td>}</tr>)}</tbody></table></div>}
    </section>
