...
function StudentStrip({name,code,roll,year}:{name:string;code:string;roll:number|null|undefined;year?:string}) { return <section className="grid gap-4 rounded-xl bg-gradient-to-r from-teal-800 to-slate-900 p-5 text-white sm:grid-cols-4"><div className="sm:col-span-2"><p className="text-[10px] uppercase text-teal-200">Student</p><p className="mt-1 font-black">{name}</p></div><div><p className="text-[10px] uppercase text-teal-200">Student ID / Roll</p><p className="mt-1 text-sm font-bold">{code} / {roll || "—"}</p></div><div><p className="text-[10px] uppercase text-teal-200">Academic year</p><p className="mt-1 text-sm font-bold">{year || "—"}</p></div></section>; }
