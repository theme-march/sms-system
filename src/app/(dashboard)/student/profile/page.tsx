...
export default async function StudentProfilePage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="flex flex-col gap-5 rounded-2xl bg-gradient-to-r from-teal-800 to-slate-900 p-6 text-white sm:flex-row sm:items-center"><div className="flex h-20 w-20 items-center justify-center rounded-2xl bg-white/10 text-2xl font-black">{student.nameEn.split(" ").map((part) => part[0]).slice(0,2).join("")}</div><div><h2 className="text-2xl font-black">{student.nameEn}</h2><p className="mt-1 text-sm text-teal-100">{student.nameBn || "Student"}</p><p className="mt-2 text-xs text-teal-200">{enrollment?.class.name || student.class?.name} · {enrollment?.section.name || student.section?.name} · Roll {enrollment?.rollNumber || student.rollNumber}</p></div><ShieldCheck className="ml-auto hidden h-7 w-7 text-teal-300 sm:block" /></section>
