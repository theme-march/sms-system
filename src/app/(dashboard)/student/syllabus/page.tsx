...
import { getPortalDownloads, normalizeWebsiteContent } from "@/src/lib/website-content";
...
export default async function StudentSyllabusPage() {
  ...
  const downloads = getPortalDownloads(
    normalizeWebsiteContent(student.school.websiteSettings?.content).downloads,
    classId,
    sectionId,
  );
  ...
  return <div className="mx-auto max-w-6xl space-y-6">
    ...
    <section className="dashboard-hero grid gap-4 rounded-xl p-5 text-white sm:grid-cols-4"><div className="sm:col-span-2"><p className="dashboard-hero-muted text-[10px] font-bold uppercase tracking-wider">Student</p><p className="mt-1 font-black">{student.nameEn}</p><p className="dashboard-hero-muted mt-1 text-xs">{student.studentCode} · Roll {enrollment?.rollNumber || student.rollNumber || "—"}</p></div><Detail label="Class / Section" value={`${className} / ${sectionName}`} /><Detail label="Academic year / Group" value={`${yearName} / ${groupName}`} /></section>
    ...
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-2xs"><h2 className="flex items-center gap-2 text-sm font-black text-slate-900"><FileText className="h-4 w-4 text-teal-700" />Uploaded syllabus files</h2><p className="mt-1 text-xs text-slate-500">Files published by the school for your class and section</p>{!downloads.length ? <p className="mt-4 rounded-xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-xs text-slate-500">No file has been uploaded for your class and section.</p> : <div className="mt-4 grid gap-3 sm:grid-cols-2">{downloads.map((item, index) => <article key={`${item.fileUrl}-${index}`} className="rounded-xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><p className="font-black text-slate-900">{item.title}</p><span className="rounded-full bg-teal-50 px-2 py-1 text-[9px] font-black text-teal-700">{item.category}</span></div><p className="mt-1 text-[11px] text-slate-500">{item.publishedAt ? `Published ${new Date(item.publishedAt).toLocaleDateString("en-GB")}` : `${className} file`}</p><div className="mt-4 flex gap-2"><a href={item.fileUrl} target="_blank" rel="noreferrer" className="btn-secondary"><Eye className="h-4 w-4" />View</a><a href={item.fileUrl} download className="btn-primary"><Download className="h-4 w-4" />Download</a></div></article>)}</div>}</section>
  </div>;
}

function Detail({label,value}:{label:string;value:string}) { return <div><p className="dashboard-hero-muted text-[10px] font-bold uppercase tracking-wider">{label}</p><p className="mt-1 text-sm font-bold">{value}</p></div>; }
