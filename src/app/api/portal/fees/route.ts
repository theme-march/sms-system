...
async function allowedStudentIds(userId: string, schoolId: string) {
  ...
  const guardian = await prisma.guardian.findFirst({
    where: { userId, schoolId, status: "ACTIVE", portalAccessEnabled: true },
    select: {
...
export async function GET(request: NextRequest) {
  ...
  if (!studentId || !ids.includes(studentId))
    return NextResponse.json(
      { error: "Student fee access is not permitted." },
      { status: 403 },
    );
  const [invoices, feeTypes, structureItems] = await Promise.all([
    prisma.feeInvoice.findMany({
      where: { schoolId: session.schoolId, studentId },
      include: {
        feeStructure: { select: { id: true, name: true, frequency: true } },
  ...
  const typeById = new Map(feeTypes.map((item) => [item.id, item.category]));
  const categoryByStructure = new Map(
    structureItems.map((item) => [
      item.feeStructureId,
      typeById.get(item.feeTypeId) || "OTHER",
    ]),
  );
  return NextResponse.json(
    toClientData(
      invoices.map((item) => {
        ...
        return {
          ...
          feeStructure: {
            ...item.feeStructure,
            category:
              categoryByStructure.get(item.feeStructure.id) ||
              (item.feeStructure.name.toUpperCase().includes("EXAM")
                ? "EXAM"
                : item.feeStructure.name.toUpperCase().includes("TUITION")
                  ? "TUITION"
                  : "OTHER"),
          },
...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success)
    return NextResponse.json(
      {
        error:
          parsed.error.issues[0]?.message || "Invalid payment information.",
      },
      { status: 400 },
    );
  ...
  if (!ids.includes(invoice.studentId))
    return NextResponse.json(
      { error: "You cannot pay this invoice." },
      { status: 403 },
    );
  const total = Number(invoice.amount) - Number(invoice.discount);
  const due = total - Number(invoice.paidAmount);
  if (invoice.status === "CANCELLED" || due <= 0)
    return NextResponse.json(
      { error: "This invoice has no payable balance." },
      { status: 409 },
    );
  if (parsed.data.amount > due)
    return NextResponse.json(
      { error: `Payment cannot exceed ${due.toFixed(2)}.` },
      { status: 400 },
    );
  ...
  const payment = await prisma.$transaction(async (tx) => {
    const record = await tx.payment.create({
      data: {
        ...
        paymentMethod: parsed.data.paymentMethod,
        transactionId: parsed.data.transactionId,
        notes: `Self-service payment by ${session.name}`,
      },
    });
    await tx.feeInvoice.update({
      where: { id: invoice.id },
      data: { paidAmount, status: status(total, paidAmount, invoice.dueDate) },
    });
    return record;
  });
  await createAuditLog({
    ...
    module: "Portal Fees",
    recordId: payment.id,
    details: `Self-service payment ${receiptNumber} for ${invoice.invoiceNumber}`,
  });
  return NextResponse.json(
    toClientData({
      receiptNumber,
      remainingDue: Math.max(0, total - paidAmount),
    }),
    { status: 201 },
  );
}
