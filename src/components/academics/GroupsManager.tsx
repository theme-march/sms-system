...
import { Search, Plus, Edit2, Trash2, CheckCircle2, XCircle, AlertCircle, Users } from 'lucide-react';
import {
  getGroupsList,
  createGroup,
  updateGroup,
...
export function GroupsManager() {
  ...
  const [editingItem, setEditingItem] = useState<GroupRecord | null>(null);
  ...
  const handleOpenCreate = () => {
    ...
    setFormData({
      name: 'Science Stream',
      description: 'Science & Technology Academic Group',
      status: 'ACTIVE',
    });
    setFormErrors({});
    setIsModalOpen(true);
  ...
  const handleOpenEdit = (item: GroupRecord) => {
    setEditingItem(item);
    setFormData({
      name: item.name,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validation = groupSchema.safeParse(payload);
    ...
    try {
      if (editingItem) {
        await updateGroup(editingItem.id, payload);
  ...
  const handleToggleStatus = async (item: GroupRecord) => {
    try {
      await toggleGroupStatus(item.id);
      await createAuditLog({
        action: 'TOGGLE_STATUS',
        module: 'Groups',
        recordId: item.id,
        details: `Toggled status for group ${item.name}`,
      });
      showNotification('Group status updated');
      loadData();
    } catch (err: any) {
  ...
  const handleDelete = async (item: GroupRecord) => {
    ...
    try {
      ...
      await createAuditLog({
        ...
        module: 'Groups',
        recordId: item.id,
        details: `Deleted group ${item.name}`,
      });
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-2xs overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center space-y-2">
            ...
            <p className="text-xs text-slate-400">Configure academic streams (e.g. Science, Commerce, Humanities).</p>
          </div>
        ) : (
          ...
          <div className="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-900">
                {editingItem ? 'Edit Academic Group' : 'Create Academic Group'}
              </h3>
