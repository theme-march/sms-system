...
import { Search, Plus, Edit2, Trash2, CheckCircle2, XCircle, AlertCircle, Layers } from 'lucide-react';
import {
  getSectionsList,
  createSection,
  updateSection,
...
export function SectionsManager() {
  ...
  const [editingItem, setEditingItem] = useState<SectionRecord | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const validation = sectionSchema.safeParse(payload);
    ...
    try {
      if (editingItem) {
        ...
        await createAuditLog({
          ...
          details: `Updated section ${formData.name}`,
        });
        ...
        await createAuditLog({
          ...
          module: 'Sections',
          recordId: created.id,
          details: `Created section ${formData.name}`,
        });
  ...
  const handleToggleStatus = async (item: SectionRecord) => {
    try {
      await toggleSectionStatus(item.id);
      await createAuditLog({
        action: 'TOGGLE_STATUS',
        module: 'Sections',
        recordId: item.id,
        details: `Toggled status for section ${item.name}`,
      });
  ...
  const handleDelete = async (item: SectionRecord) => {
    ...
    try {
      ...
      await createAuditLog({
        ...
        module: 'Sections',
        recordId: item.id,
        details: `Deleted section ${item.name}`,
      });
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 shadow-2xs overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center space-y-2">
            ...
            <p className="text-xs text-slate-400">Define section templates for classrooms and student grouping.</p>
          </div>
        ) : (
          ...
          <div className="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md overflow-hidden">
            <div className="px-5 py-4 border-b border-slate-100 flex items-center justify-between">
              <h3 className="text-sm font-bold text-slate-900">
                {editingItem ? 'Edit Section' : 'Create Section'}
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Maximum Student Capacity</label>
                <input
                  type="number"
